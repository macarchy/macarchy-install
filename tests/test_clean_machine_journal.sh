...
check "(c) not the notifier itself"   bash -c '! grep -q "MARKER body of macarchy-failed@" <<<"$1"' _ "$out"
...
./tests/dump-failed-units.sh >/dev/null 2>&1
check "(e) survives a journalctl failure" [ "$?" = 0 ]
