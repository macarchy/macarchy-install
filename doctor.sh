...
echo "binaries"
for b in omarchy-dfr omarchy-als omarchy-battery-limit omarchy-dock \
...
[[ -f /etc/tiny-dfr/close.svg ]] \
	&& ok "Touch Bar icons in /etc/tiny-dfr" || bad "Touch Bar icons in /etc/tiny-dfr"
systemctl is-active -q tiny-dfr 2>/dev/null \
	&& ok "tiny-dfr service" || bad "tiny-dfr service (pacman -S tiny-dfr; systemctl enable --now tiny-dfr)"
...
grep -qF "omarchy-dfr press"        "$BIND" 2>/dev/null && ok "Touch Bar binds"    || bad "Touch Bar binds"
...
grep -qF "omarchy-dfr daemon"       "$AUTO" 2>/dev/null && ok "dfr autostart"      || bad "dfr autostart"
...
if [[ -n ${HYPRLAND_INSTANCE_SIGNATURE:-} ]]; then
	echo "running now"
	pgrep -f "omarchy-dfr daemon" >/dev/null && ok "omarchy-dfr"   || bad "omarchy-dfr not running"
	...
	omarchy-aquarium-toggle status >/dev/null 2>&1 && ok "aquarium" || bad "aquarium not running (SUPER+ALT+A)"
fi
